...
export default function App() {
  ...
  // Load all tournaments from Supabase
  const loadData = async () => {
    ...
    try {
      ...
      // Parse the JSON data stored in batting/pitching columns
  ...
  const createTournament = async () => {
    ...
    showNotification(`Tournament "${newTournament.name}" created!`);
  };
  ...
  const deleteTournament = async (id) => {
    ...
    try {
      ...
      setTournaments(updated);
      setSelectedTournament(newSelected);
      if (newSelected) {
        localStorage.setItem('selectedTournamentId', newSelected.id);
      } else {
    ...
    newPlayers.forEach(p => {
      if (playerMap.has(p.name)) {
        ...
        if (type === 'pitching') {
          ...
          const combinedG = existing.g + p.g;
          const combinedGS = existing.gs + p.gs;
          const combinedBF = existing.bf + p.bf;
          
          const existingIP = parseIP(existing.ip);
          const newIP = parseIP(p.ip);
          
          const weightedAvg = (stat1, ip1, stat2, ip2) => {
            if (ip1 + ip2 === 0) return 0;
            return ((parseFloat(stat1) * ip1) + (parseFloat(stat2) * ip2)) / (ip1 + ip2);
          };
          ...
          const combinedG = existing.g + p.g;
          const combinedGS = existing.gs + p.gs;
          const combinedPA = existing.pa + p.pa;
          const combinedAB = existing.ab + p.ab;
          const combinedH = existing.h + p.h;
          const combined2B = existing.doubles + p.doubles;
          const combined3B = existing.triples + p.triples;
          const combinedHR = existing.hr + p.hr;
          const combinedRBI = existing.rbi + p.rbi;
          const combinedR = existing.r + p.r;
          const combinedBB = existing.bb + p.bb;
          const combinedIBB = existing.ibb + p.ibb;
          const combinedHP = existing.hp + p.hp;
          const combinedSO = existing.so + p.so;
          const combinedGIDP = existing.gidp + p.gidp;
          const combinedSB = existing.sb + p.sb;
          const combinedCS = existing.cs + p.cs;
          
          const avg = combinedAB > 0 ? (combinedH / combinedAB).toFixed(3) : '.000';
          const obp = combinedPA > 0 ? ((combinedH + combinedBB + combinedHP) / combinedPA).toFixed(3) : '.000';
          const tb = combinedH + combined2B + (2 * combined3B) + (3 * combinedHR);
          const slg = combinedAB > 0 ? (tb / combinedAB).toFixed(3) : '.000';
          const ops = (parseFloat(obp) + parseFloat(slg)).toFixed(3);
          const iso = combinedAB > 0 ? ((tb - combinedH) / combinedAB).toFixed(3) : '.000';
          
          const combinedOPSplus = combinedPA > 0 ? 
            ((existing.opsPlus * existing.pa) + (p.opsPlus * p.pa)) / combinedPA : 0;
          
          const babipNum = combinedH - combinedHR;
          const babipDenom = combinedAB - combinedSO - combinedHR;
          ...
          playerMap.set(p.name, {
            ...
            iso: iso,
            ops: ops,
            opsPlus: Math.round(combinedOPSplus),
            babip: babip,
            war: (parseFloat(existing.war || 0) + parseFloat(p.war || 0)).toFixed(1),
            sb: combinedSB,
            cs: combinedCS
        ...
        playerMap.set(p.name, { ...p, id: crypto.randomUUID() });
      }
    });
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        ...
        const processedData = results.data
          ...
          .map(row => normalizePlayerData(row, uploadType));
  ...
  const getFilteredData = (data, type) => {
    ...
    if (filters.position !== 'all') {
      filtered = filtered.filter(p => p.pos.toUpperCase() === filters.position.toUpperCase());
    }
    ...
    if (type === 'batting') {
      ...
      filtered = filtered.filter(p => passesFilter(parseIP(p.ip), filters.ipFilter));
    }
    filtered.sort((a, b) => {
  ...
  const updateStatFilter = (filterName, updates) => {
    setFilters(f => ({ ...f, [filterName]: { ...f[filterName], ...updates } }));
  };
  ...
  const getActiveFilterCount = () => {
    ...
    if (filters.gFilter.enabled) count++;
    if (filters.paFilter.enabled) count++;
    if (filters.abFilter.enabled) count++;
    if (filters.ipFilter.enabled) count++;
    return count;
  };
  ...
  const getHandednessStats = (players, field) => {
    ...
    const counts = { L: 0, S: 0, R: 0 };
    players.forEach(p => {
      const val = (p[field] || '').toUpperCase();
      if (val === 'L') counts.L++;
      else if (val === 'S') counts.S++;
      else if (val === 'R') counts.R++;
    });
    return {
      L: ((counts.L / total) * 100).toFixed(0),
      S: ((counts.S / total) * 100).toFixed(0),
      R: ((counts.R / total) * 100).toFixed(0)
    };
  };
        ...
        <aside style={styles.sidebar}>
          ...
          {showNewTournament && (
            <div style={styles.newForm}>
              <input type="text" placeholder="Tournament name..." value={newTournamentName}
          ...
          <div style={styles.tournamentList}>
            {tournaments.length === 0 ? <p style={styles.emptyMsg}>No tournaments yet</p> :
              tournaments.map(t => (
                <div key={t.id} style={{...styles.tournamentItem, ...(selectedTournament?.id === t.id ? styles.tournamentActive : {})}}
          ...
          {!selectedTournament ? (
            ...
            <>
                ...
                {(selectedTournament.pitching.length > 0 || selectedTournament.batting.length > 0) && (
                  <div style={styles.handednessContainer}>
                    {selectedTournament.pitching.length > 0 && (() => {
                      const pitchStats = getHandednessStats(selectedTournament.pitching, 'throws');
                      return (
                        <div style={styles.handednessGroup}>
                          <span style={styles.handednessLabel}>Pitchers (T):</span>
                          <span style={styles.handednessValue}>L {pitchStats.L}%</span>
                          <span style={styles.handednessValue}>S {pitchStats.S}%</span>
                          <span style={styles.handednessValue}>R {pitchStats.R}%</span>
                        </div>
                      );
                    })()}
                    {selectedTournament.batting.length > 0 && (() => {
                      const batStats = getHandednessStats(selectedTournament.batting, 'bats');
                      return (
                        <div style={styles.handednessGroup}>
                          <span style={styles.handednessLabel}>Batters (B):</span>
                          <span style={styles.handednessValue}>L {batStats.L}%</span>
                          <span style={styles.handednessValue}>S {batStats.S}%</span>
                          <span style={styles.handednessValue}>R {batStats.R}%</span>
                        </div>
                      );
                    })()}
                  </div>
              ...
              <div style={styles.uploadSection}>
                ...
                <span style={styles.uploadHint}>Players with same name = stats combined</span>
              </div>
              <div style={styles.tabs}>
                <button style={{...styles.tab, ...(activeTab === 'pitching' ? styles.tabActive : {})}}
                  onClick={() => { setActiveTab('pitching'); setFilters(f => ({...f, position: 'all'})); }}>
                  ⚾ Pitching ({selectedTournament.pitching.length})
                </button>
                <button style={{...styles.tab, ...(activeTab === 'batting' ? styles.tabActive : {})}}
                  onClick={() => { setActiveTab('batting'); setFilters(f => ({...f, position: 'all'})); }}>
                  🏏 Batting ({selectedTournament.batting.length})
                </button>
              </div>
              <div style={styles.filterBar}>
                <input type="text" placeholder="Search..." value={filters.search}
                  onChange={(e) => setFilters(f => ({...f, search: e.target.value}))} style={styles.searchInput} />
                <select value={filters.position} onChange={(e) => setFilters(f => ({...f, position: e.target.value}))} style={styles.filterSelect}>
                  {(activeTab === 'pitching' ? pitchingPositions : battingPositions).map(pos => (
                    <option key={pos} value={pos}>{pos === 'all' ? 'All Positions' : pos}</option>
                  ))}
                </select>
                <button 
                  style={{...styles.advancedFilterBtn, ...(showAdvancedFilters ? styles.advancedFilterBtnActive : {}), ...(getActiveFilterCount() > 0 ? styles.advancedFilterBtnHasFilters : {})}}
              ...
              {showAdvancedFilters && (
                <div style={styles.advancedFilters}>
                  <div style={styles.filterGroup}>
                    <StatFilter label="Games (G)" filter={filters.gFilter} onChange={(updates) => updateStatFilter('gFilter', updates)} />
                    {activeTab === 'batting' ? (
                      <>
                        <StatFilter label="Plate Appearances (PA)" filter={filters.paFilter} onChange={(updates) => updateStatFilter('paFilter', updates)} />
                        <StatFilter label="At Bats (AB)" filter={filters.abFilter} onChange={(updates) => updateStatFilter('abFilter', updates)} />
                      </>
                    ) : (
                      <StatFilter label="Innings Pitched (IP)" filter={filters.ipFilter} onChange={(updates) => updateStatFilter('ipFilter', updates)} />
                    )}
                  </div>
                </div>
              )}
              ...
              <div style={styles.tableContainer}>
                {activeTab === 'pitching' ? (
                  <PitchingTable data={filteredData} sortBy={filters.sortBy} sortDir={filters.sortDir} onSort={toggleSort} />
                ) : (
                  <BattingTable data={filteredData} sortBy={filters.sortBy} sortDir={filters.sortDir} onSort={toggleSort} />
                )}
              </div>
            </>
          )}
...
function StatFilter({ label, filter, onChange }) {
  return (
    <div style={styles.statFilter}>
      <label style={styles.statFilterLabel}>
        <input type="checkbox" checked={filter.enabled} onChange={(e) => onChange({ enabled: e.target.checked })} style={styles.checkbox} />
        {label}
      </label>
...
function PitchingTable({ data, sortBy, sortDir, onSort }) {
  ...
  const calcIPperG = (ip, g) => {
    if (!g || g === 0) return '0.00';
    let ipNum = 0;
    const ipStr = String(ip);
    if (ipStr.includes('.')) {
      const [whole, frac] = ipStr.split('.');
      ipNum = parseFloat(whole) + (parseFloat(frac) / 3);
    } else {
      ipNum = parseFloat(ip) || 0;
    }
    return (ipNum / g).toFixed(2);
  };
  ...
  return (
    <table style={styles.table}>
      <thead><tr>
        ...
        <SortHeader field="winPct">WIN%</SortHeader>
        <SortHeader field="svPct">SV%</SortHeader>
      ...
      <tbody>
        {data.map(p => (
          <tr key={p.id} style={styles.tr}>
            <td style={styles.tdPos}>{p.pos}</td>
            <td style={styles.tdName}>{p.name}</td>
            <td style={styles.tdNum}>{p.throws}</td>
            <td style={styles.tdNum}>{p.g}</td>
            <td style={styles.tdNum}>{p.gs}</td>
            <td style={styles.tdNum}>{p.winPct}</td>
            <td style={styles.tdNum}>{p.svPct}</td>
            <td style={styles.tdNum}>{p.ip}</td>
            <td style={styles.tdStat}>{calcIPperG(p.ip, p.g)}</td>
            <td style={styles.tdNum}>{p.bf}</td>
            ...
            <td style={styles.tdNum}>{p.lobPct}</td>
            ...
            <td style={styles.tdStat}>{p.siera}</td>
          </tr>
        ))}
      </tbody>
    </table>
  ...
  return (
    <table style={styles.table}>
      <thead><tr>
        ...
        <SortHeader field="rbi">RBI</SortHeader>
        <SortHeader field="r">R</SortHeader>
        <SortHeader field="bb">BB</SortHeader>
        <SortHeader field="so">SO</SortHeader>
        <SortHeader field="avg">AVG</SortHeader>
        <SortHeader field="obp">OBP</SortHeader>
        <SortHeader field="slg">SLG</SortHeader>
        <SortHeader field="ops">OPS</SortHeader>
        <SortHeader field="iso">ISO</SortHeader>
        <SortHeader field="opsPlus">OPS+</SortHeader>
        <SortHeader field="babip">BABIP</SortHeader>
        <SortHeader field="war">WAR</SortHeader>
        <SortHeader field="sb">SB</SortHeader>
        <SortHeader field="cs">CS</SortHeader>
      </tr></thead>
      <tbody>
        {data.map(p => (
          <tr key={p.id} style={styles.tr}>
            <td style={styles.tdPos}>{p.pos}</td>
            <td style={styles.tdName}>{p.name}</td>
            <td style={styles.tdNum}>{p.bats}</td>
            <td style={styles.tdNum}>{p.g}</td>
            <td style={styles.tdNum}>{p.gs}</td>
            <td style={styles.tdNum}>{p.pa}</td>
            <td style={styles.tdNum}>{p.ab}</td>
            <td style={styles.tdNum}>{p.h}</td>
            <td style={styles.tdNum}>{p.doubles}</td>
            <td style={styles.tdNum}>{p.triples}</td>
            <td style={styles.tdNum}>{p.hr}</td>
            <td style={styles.tdNum}>{p.rbi}</td>
            <td style={styles.tdNum}>{p.r}</td>
            <td style={styles.tdNum}>{p.bb}</td>
            <td style={styles.tdNum}>{p.so}</td>
            <td style={styles.tdStat}>{p.avg}</td>
            <td style={styles.tdStat}>{p.obp}</td>
            <td style={styles.tdStat}>{p.slg}</td>
            <td style={styles.tdStat}>{p.ops}</td>
            <td style={styles.tdStat}>{p.iso}</td>
            <td style={styles.tdStat}>{p.opsPlus}</td>
            <td style={styles.tdStat}>{p.babip}</td>
            <td style={{...styles.tdStat, color: parseFloat(p.war) >= 0 ? '#4ade80' : '#f87171'}}>{p.war}</td>
            <td style={styles.tdNum}>{p.sb}</td>
            <td style={styles.tdNum}>{p.cs}</td>
          </tr>
        ))}
      </tbody>
    </table>
